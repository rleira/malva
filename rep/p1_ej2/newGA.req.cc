...
skeleton newGA
{
	...
	Problem::Problem ():_dimension(0),_max_peso(0),_peso_item(NULL),_ganancia_item(NULL),_input_file("")
	{}
	...
	ostream& operator<< (ostream& os, const Problem& pbm)
	{
		...
		//Imprimo el arreglo con los valores de la lista de la mochila
		os<<"Valores: "<<endl<<endl;
		for (int i=0;i<pbm._dimension;i++){
			os<<i<<": "<<pbm._ganancia_item[i]<<": "<<pbm._peso_item[i];
			os<<endl;
		}
		os<<endl;
	...
	istream& operator>> (istream& is, Problem& pbm)
	{
		...
		// Obtengo nombre de archivo que contiene los datos de la mochila
		is.getline(buffer,MAX_BUFFER,'\n');

		sscanf(buffer," %s",pbm._input_file);

		// Abro archivo con datos
		FILE* stream = fopen(pbm._input_file, "r");
		char line[1024];
	    ...
	    pbm._dimension=atoi(item);

		//Pido memoria para almacenar los pesos y ganancias de la mochila
		pbm._peso_item=new float [pbm._dimension];
		pbm._ganancia_item=new float [pbm._dimension];
		for (int i = 0; (i < pbm._dimension && fgets(line, 1024, stream)); i++){
	        ...
	        // Obtengo ganancia para el item
	        tmp = strdup(line);
	        const char * ganancia= pbm.getfield(tmp,2);
	        pbm._ganancia_item[item_int-1]=atof(ganancia);
			free(tmp);

	        // Obtengo peso del item
	        tmp = strdup(line);
	        const char * peso= pbm.getfield(tmp,3);
	        pbm._peso_item[item_int-1]=atof(peso);
	        free(tmp);

	        cout << item << endl;
	        cout << pbm._ganancia_item[item_int-1] << endl;
	        cout << pbm._peso_item[item_int-1] << endl;

	        
		}

	    // Obtengo max peso de la mochila
		fgets(line, 1024, stream);
		tmp = strdup(line);
		const char * maxPeso = pbm.getfield(tmp,1);
	    pbm._max_peso=atof(maxPeso);
	...
	float * Problem::peso_item() const{
		return _peso_item;
	}

	float * Problem::ganancia_item() const{
		return _ganancia_item;
	}

	float Problem::max_peso() const{
		return _max_peso;
	}
	...
	double Solution::fitness ()
	{
		...
		for (int i=0; i<_var.size(); i++){
			fitness += _var[i] * pbm().ganancia_item()[i];
			peso += _var[i] * pbm().peso_item()[i];
		}

		penalty = fmin(0.0f, pbm().max_peso()*fabs(pbm().max_peso() - peso)*(pbm().max_peso() - peso));
		fitness += penalty;
...
//  Crossover:Intra_operator -------------------------------------------------------------
	...
	void Mutation::mutate(Solution& sol) const
	{
		for (int i=0;i<sol.pbm().dimension();i++)
		{
			...
			{
				//La mutacion modifica un gen aleatoriamente con probabilidad uniforme en {0,7}
				sol.var(i)=rand_int(0,1);
			}
		}
	}
...
// StopCondition_1 -------------------------------------------------------------------------------------
	...
	bool StopCondition_1::EvaluateCondition(const Problem& pbm,const Solver& solver,const SetUpParams& setup)
	{
		...
		for (int i=0;i<pbm.dimension();i++){
			ganancia += solver.best_solution_trial().var(i) * pbm.ganancia_item()[i];
			peso += solver.best_solution_trial().var(i) * pbm.peso_item()[i];
		}
		//Condicion de parada. Si llegamos al peso indicado
		bool sol = (peso <= pbm.max_peso());
		if (sol){
			//Escribo el resultado en el archivo de salida
			FILE * pFile;
			pFile = fopen ("p1_ej2_output.csv","w");

			for (int i=0;i<pbm.dimension()-1;i++){
				fprintf (pFile, "%i,",solver.best_solution_trial().var(i));
			}

			fprintf (pFile, "%i\n",solver.best_solution_trial().var(pbm.dimension()-1));
			fprintf (pFile, "%f\n",ganancia);
			fprintf (pFile, "%f\n",peso);
