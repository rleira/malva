...
skeleton newGA
{
	...
	Problem::Problem ():_dimension(0),_max_peso(0),_peso_item(NULL)
	{}
	...
	istream& operator>> (istream& is, Problem& pbm)
	{
		...
		is.getline(buffer,MAX_BUFFER,'\n');
		sscanf(buffer,"%f",&pbm._max_peso);

		//Pido memoria para almacenar los valores de la mochila
		pbm._peso_item=new float [pbm._dimension];
		for (int i=0;i<pbm._dimension;i++){
			pbm._peso_item[i]=0.0f;
		}

		// Cargo pesos de los items
		FILE* stream = fopen("p1_ej1_input.csv", "r");
		char line[1024];
	    ...
	    while (fgets(line, 1024, stream))
	    {
			...
			tmp = strdup(line);	
	        const char * peso= pbm.getfield(tmp,2);
	...
	double Solution::fitness ()
	{
		...
		for (int i=0; i<_var.size(); i++){
			fitness += _var[i] * pbm().peso_item()[i];
		}

		fitness = fabs(pbm().max_peso() - fitness);
...
// StopCondition_1 -------------------------------------------------------------------------------------
	...
	bool StopCondition_1::EvaluateCondition(const Problem& pbm,const Solver& solver,const SetUpParams& setup)
	{
		...
		bool fin=(float)solver.best_cost_trial() == 0.0f;
		if (fin){
			//Escribo el resultado en el archivo de salida
			FILE * pFile;
			pFile = fopen ("p1_ej1_output.csv","w");
			for (int i=0;i<pbm.dimension();i++){
				fprintf (pFile, "%d,%d,%f\n",i+1,solver.best_solution_trial().var(i), pbm.peso_item()[i]);
			}
			fclose (pFile);
		}
		return (fin);
